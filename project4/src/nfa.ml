...
let rm newqs eacc acc = match newqs with
...
let accept (nfa: ('q,char) nfa_t) (s: string) : bool =
  let rec check str states = match str with
    | h::t -> let states2 = e_closure nfa states in let moves = move nfa states2 (Some h)
...
let chop dfa nfa = if finalize (List.hd dfa.fs) nfa.fs then {dfa with fs = removedup(dfa.fs) []} else {dfa with fs = removedup(List.tl dfa.fs) []}
;;

let nfa_to_dfa (nfa: ('q,'s) nfa_t) : ('q list, 's) nfa_t =
  let kruskal = List.sort (compare) (e_closure nfa [nfa.q0]) in
  let dfa = {
    sigma = nfa.sigma;
    qs = [kruskal];
    q0 = kruskal;
  ...
  in chop(nfa_to_dfa_step nfa dfa dfa.qs) nfa
;;
