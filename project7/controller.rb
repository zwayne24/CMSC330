...
module Access
	...
	def authenticate(user, passwd)
		assign_session user if get_both(user,passwd)
	end
...
module User
        ...
        def search(user_query)
		      ...
		      Description LIKE ?
                ...
                @db.execute(query,[user_query,user_query]) do |user|
                        users << {
                                :name => user[0],
                                :avatar => user[1],
                                :description => user[2]
	...
	def register(user, filename, file, password, confirm)
		...
		upload_avatar(filename, file)
			query = %{
				...
				SELECT User,Password,Avatar
				FROM (SELECT ? as User, ? as Password, ? as Avatar) t
				WHERE NOT EXISTS (SELECT 1 FROM Users WHERE Users.user = t.user)
			}
			@db.execute(query,[user,password,filename])
			true
	...
	def get_both(user,passwd)
		...
		@db.execute(query,[user, passwd]) do |user|
			return {
				:avatar => user[0],
				:description => user[1]
			}
	...
	def update_prefs(user, session, description, token)
		...
		if /<+(?<mid>.*)>*/ =~ description
			ret = false
		elsif /(http:)+(.*)/ =~ description
			ret = false
		elsif description.include? "&" || description.include?("\"") ||  description.include?("\'")
			ret = false
		end
		if token == @tokens[user] && session == @sessions[user] && ret
			query = %{
				UPDATE Users
				SET Description = ?
				WHERE User = ?
...
module Posts
	...
	def publish_post(user, session, content, token)
		ret = true
		if token == @tokens[user] && session == @sessions[user]
